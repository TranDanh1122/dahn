...
router.get("/sse", async (req, res) => {
  ...
  res.flushHeaders();
  //at a time, server can have multiple connect
  projectSSEClients.push(res);
  req.on("close", () => {
    res.end();
  });
});

const pushSSE = (newData) => {
  projectSSEClients.forEach((res) => {
...
router.delete("/:projectId", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Project deleted", data: project });
  } catch (e) {
    return res.status(500).json({
      success: false,
...
const checkRole = async (req, asset, role) => {
  ...
  if (!isOwner && !matchRole)
    throw new Error("You dont have permission to edit this Project");
};
router.put("/:projectId/general", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Project Edited", data: updatedProject });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: error.message || "Internal server error",
