...
export default function PermissionItem({ data }: PermissionItemProps): React.JSX.Element {
    return <>
        <div className="space-y-1">
            {data?.value == "admin" && <UserCog className="size-4 text-neutral-500 " />}
            {data?.value == "write" && <SquarePen className="size-4 text-neutral-500 " />}
            {data?.value == "read" && <PencilOff className="size-4 text-neutral-500 " />}
            <h2 className="text-neutral-600 text-sm font-medium">{data?.text}</h2>
