        ...
        <div className="grid grid-cols-5 gap-10 auto-rows-[230px]">
            ...
            {!isLoading && data && data.length > 0 &&
                data.slice(0, Math.max(data.length - 1, 3)).map(el => {
                    return <SquareItem key={el.id} img={el.image} title={el.name} lastUpdate={el.created_at} />
                })
            }

            {!isLoading && <ListItemButton onClick={() => navigate("/workspace/create")} title="Add new workspace" >
