        ...
        <fieldset className={`space-y-2 ${fieldsetClass}`}>
            {
                label &&
                <label
                    className={`font-semibold text-slate-600 cursor-pointer text-sm ${labelClass}`}
                    htmlFor={props.id}
                    aria-label={label}>
                    {label}
                </label>
            ...
            <div className="relative w-full">
                <input {...props}
                    ...
                    className={clsx(
                        `placeholder:text-sm text-sm w-full cursor-pointer 
                        border border-slate-200 font-medium px-3 py-2 rounded-xl
                        focus-visible:outline-0 focus-visible:border-blue-300`,
                        className,
                        props.type && "pr-10",
                    )} />
                {
                    props.type == "password" &&
                    <div
                        className="size-5 text-slate-400 top-1/2 right-4 -translate-y-1/2 absolute"
                        onClick={() => setShowPass((val: boolean) => !val)}>
                        {isShowPass && <Eye />}
