...
export default React.memo(function TextArea({ labelClass, error, label, children, className, ...props }: TextAreaProps): React.JSX.Element {
    return <fieldset className="flex flex-col gap-2">
        <label className={`font-semibold text-slate-600 cursor-pointer ${labelClass}`} htmlFor={props.id} aria-label={label} >
            {label}
        </label>
        <div className="relative w-full">
            <textarea {...props} className={` ${className} w-full cursor-pointer border border-slate-200 font-medium px-3 py-2 focus-visible:outline-0 focus-visible:border-blue-300 rounded-xl`} />
            {children && children}
        </div>
