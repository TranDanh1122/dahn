...
const InputSearch = <T, K>({ filter, childrenFn, searchServiceFn, resultItemClick, ...props }: InputSearchProps<T, K>): React.JSX.Element => {
    const { searching, setSearching, ref, itemClick } = useInputSearch<T, K>({ resultItemClick })
    return <div ref={ref} className="relative w-2/3">
        ...
        {searching && <DropdownSearch filter={filter} childrenFn={childrenFn} searchServiceFn={searchServiceFn} resultItemClick={itemClick} searching={searching} />}
    </div>
}
